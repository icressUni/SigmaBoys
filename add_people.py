...
def gui_main():
    def start_capture():
        ...
        if option_var.get() == "1":
            ...
            import_image_and_save(folder_path)
        else:
            status_label.config(text="Error: Please select a valid option.")
    ...
    Label(root, text="Choose an option:").grid(row=1, column=0, padx=10, pady=5)
    option_var = StringVar(value="1")
    Radiobutton(root, text="Take Pictures", variable=option_var, value="1").grid(row=1, column=1, padx=10, pady=5)
    Radiobutton(root, text="Import Image", variable=option_var, value="2").grid(row=1, column=2, padx=10, pady=5)

    Button(root, text="Start", command=start_capture).grid(row=2, column=1, padx=10, pady=10)

    status_label = Label(root, text="Please enter your name to create a folder before taking pictures.")
    ...
    video_canvas.grid(row=4, column=0, columnspan=3, pady=10)
